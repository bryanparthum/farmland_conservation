...
design_matrix <- cbind(design_matrix_miles, (design_matrix_minutes %<>% select(minutes))) %>% relocate(minutes, .after = miles)
...
We first estimate the model using a simple multinomial logit (MNL). We then introduce the random parameter model (MXL) with uncorrelated random parameters. We maintain the assumption that parameters are uncorrelated to ease estimation during the simulation, but it is reasonable to relax this assumption to recover a full covariance matrix. We assume the distribution on _cost_ is fixed in order to derive subsequent empirical distributions of willingness to pay (WTP) using the a krinsky-robb bootstrap method. This means that the marginal utility of money is uniform across respondents. Finally, we add interactions between _distance_ and _nature_, _farmland_, _meals^{nature}_, and _meals^{farm}_.
