...
Note: the parameters associated with the interaction terms are assumed fixed across respondents for ease of estimation. 

$$
\begin{aligned}
V_{it} = & \ ASC + \lambda cost_{it} + \beta^1_{i} nature_{it} + \beta^2_{i} farmland_{it} + \beta^3_{i} meals^{nature}_{it} + \beta^4_{i} meals^{farm}_{it} + \beta^5_{i} distance_{it} \\ 
...
Here, the parameter on cost ($\lambda$) is assumed to be linear (implying a constant marginal utility of income) while the other parameters correspond to each attribute's level $n$. During this simulation, the only nonlinear parameters that could be estimated were $\beta^1_n$ and $\beta^2_n$, the parameters associated with $nature$ and $farmland$. Attempting to estimate the others (either independantly or jointly) resulted in a computationally singular matrix (not estimable). Perhaps this has to do with the constraints imposed on the design? In particular, the $meals$ attributes are constrained to be positive only when the $acres$ attributes are also positive. This is a strong condition to impose on the design. Prior to imposing this condition, all $\beta^i_n$ parameters were estimable. Perhaps it would be worth thinking about which parameters are most relevant to estimate nonlinearly. $Acres$? $Distance$? It is also entirely possible that this could be remedied with real choice data and a reasonable sample size.

$$V_{it} = ASC + \lambda cost_{it} + \sum_n \beta^1_{n} nature_{it} + \sum_n \beta^2_{n} farmland_{it} + \beta^3 meals^{nature}_{it} + \beta^4 meals^{farm}_{it} + \beta^5 distance_{it} + \varepsilon_{it}$$

```{r message=FALSE, warning=FALSE, eval=TRUE, echo=TRUE}

pref_factor <-  gmnl(choice ~ asc + cost + as.factor(nature) + as.factor(farmland) + meals_nature + meals_farmland + distance | 0 ,
                    data=d,
                    model='mnl')
